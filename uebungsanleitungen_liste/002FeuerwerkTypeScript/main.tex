...
\begin{document}
    ...
    \begin{center}
    \texttt{fireworks.push(new Firework(60, (p.width * 2) / 4, 75, 25));}
    \end{center}
    ...
    Deine Aufgabe ist, mit den Parametern zu experimentieren. Hier ein paar Hinweise dazu:
        \begin{itemize}
            ...
            \item Ändere den ersten Parameter \textit{60} auf einen anderen Wert zwischen \textit{0} und \textit{360}. Hier siehst du eine Darstellung, welcher Wert zu welcher Farbe wird:

            \includegraphics[width=10cm]{images/Farben.png}
            
            \item Beim zweiten Parameter \textit{p.width / 2} wird die Mitte des Bildschirms errechnet. Du kannst hier statt der Formel auch eine Zahl angeben. Je kleiner die Zahl, desto weiter links erscheint die Rakete, je größer desto weiter rechts. Probiere mal 20, 200 und 400 aus. Siehst du, wie sich die Position der Rakete ändert?
            \item Der dritte Parameter 75 steuert, wie hoch die Rakete fliegen wird. 0 bedeutet, dass sie kaum abhebt und 100 bedeutet, dass sie bis zum oberen Bildschirmrand fliegen wird. Probiere mal 0, 50, 75 und 100 aus. Siehst du, wie sich die Höhe der Rakete ändert
            \item Der vierte Parameter 25 bestimmt die Größe der Explosion. Probiere mal 10, 15 und 25 aus.
        \end{itemize}
    }
    ...
    \begin{tcolorbox}[colback=gray!30, colframe=white]
        \begin{verbatim}
fireworks.push(new Firework(60, p.width * 0 / 4, 75, 15));
fireworks.push(new Firework(60, p.width * 1 / 4, 75, 15));
fireworks.push(new Firework(60, p.width * 2 / 4, 75, 15));
fireworks.push(new Firework(60, p.width * 3 / 4, 75, 15));
fireworks.push(new Firework(60, p.width * 4 / 4, 75, 15));\end{verbatim}
    \end{tcolorbox}
    ...
    \begin{tcolorbox}[colback=gray!30, colframe=white]
        ...
        \vspace{-0.4cm}
        \begin{verbatim}
fireworks.push(new Firework(color, p.width * 0 / 4, 75, 15));
fireworks.push(new Firework(color, p.width * 1 / 4, 75, 15));
fireworks.push(new Firework(color, p.width * 2 / 4, 75, 15));
fireworks.push(new Firework(color, p.width * 3 / 4, 75, 15));
fireworks.push(new Firework(color, p.width * 4 / 4, 75, 15));\end{verbatim}
    \end{tcolorbox}
    \vspace{0.2cm}
...
const color = 180;\end{verbatim}
        ...
        \vspace{-0.4cm}
        \begin{verbatim}
fireworks.push(new Firework(color, p.width * 0 / 4, height, size));
fireworks.push(new Firework(color, p.width * 1 / 4, height, size));
fireworks.push(new Firework(color, p.width * 2 / 4, height, size));
fireworks.push(new Firework(color, p.width * 3 / 4, height, size));
fireworks.push(new Firework(color, p.width * 4 / 4, height, size));\end{verbatim}
    \end{tcolorbox}
  ...
  fireworks.push(new Firework(color, p.width * 2 / 4, height, size));
  fireworks.push(new Firework(color, p.width * 3 / 4, height, size));
  fireworks.push(new Firework(color, p.width * 4 / 4, height, size));
  await delay(1000);
}\end{verbatim}
    \end{tcolorbox}
    ...
    \TextDesign{
    Hmmm, könnten wir die \textit{for}-Schleife nicht auch verwenden, um die vielen, kopierten Aufrufe von \textit{fireworks.push} zu vereinfachen. Deine Aufgabe ist es, eine \textit{for}-Schleife zu verwenden, um aus den fünf \textit{fireworks.push}-Aufrufen einen zu machen. So muss der Code nach der Änderung aussehen:
    }
...
for (let i = 0; i < 10; i++) {
  ...
  const size = 15; \end{verbatim}
        ...
        \hspace{0.7cm}
        \texttt{\textcolor{red}{fireworks.push(new Firework(color, (p.width * j) / 4, height, size)); }} \\
        \hspace{0.3cm}
        \texttt{\textcolor{red}{\}}} \\
        \vspace{-0.4cm}
        \begin{verbatim}
...
for (let i = 0; i < 10; i++) {
  ...
  const size = 15; \end{verbatim}
        ...
        \hspace{0.7cm}
        \texttt{\textcolor{red}{fireworks.push(new Firework(color, (p.width * j) / (numberOfRockets - 1),}} \\
        \hspace{1.5cm}
        \texttt{\textcolor{red}{height, size));}} \\
        \hspace{0.3cm}
        \texttt{\textcolor{red}{\}}} \\
        \vspace{-0.4cm}
        \begin{verbatim}
